...
If a labelled poset $P = ([n], <_P)$ is totally ordered (i.e., $<_P$ is total), it can be described by a permutation $\pi \in S_n$. In this case, we give the $\Gamma_\mathcal{Z}([n], <_P, \epsilon)$ a new name:
...
Thus, by \eqref{eq.prop.Gamma.linexts.eq}, each $\Gamma_\mathcal{Z}([n], <_P, \epsilon)$ is a sum of $U^\mathcal{Z}_{\pi,\alpha}$s.
...
\end{equation*}  
Furthermore, if $I\subseteq [2, n-1]$ is a set containing no two consecutive integers, then we set
\[
