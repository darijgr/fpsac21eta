...
\usepackage[paperwidth=21cm,paperheight=232cm,margin=1in]{geometry}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{array}
\usepackage{setspace}
...
\url{https://www.cip.ifi.lmu.de/~grinberg/algebra/fps21eta-talk.pdf}}%
...
\end{gather*}
\item The families $\tup{M_\al}_{n\in \NN,\ \al\in \Comp(n)}$
and $\tup{L_\al}_{n\in \NN,\ \al\in \Comp(n)}$
...
\item When $\alpha$ is odd, this recover's Hsiao's monomial
peak $\eta_\alpha$ up to sign.
...
The product formula \eqref{eq : EEE} generalizes a result of Hsiao \cite{Hsi07}.
